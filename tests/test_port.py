...
def test_single_port_found(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "brasa.core.port.glob.glob",
        lambda p: ["/dev/cu.usbserial-1"] if "usbserial" in p else [],
    )
    assert detect_port() == "/dev/cu.usbserial-1"


def test_multiple_ports_warns_and_returns_first(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    ports = ["/dev/cu.usbserial-1", "/dev/cu.usbserial-2"]
    monkeypatch.setattr(
        "brasa.core.port.glob.glob", lambda p: ports if "usbserial" in p else []
    )
    result = detect_port()
...
def test_custom_patterns(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "brasa.core.port.glob.glob", lambda p: ["/dev/ttyUSB0"] if "ttyUSB" in p else []
    )
...
def test_fallback_known_vendor_suggests_driver(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    data = {"SPUSBDataType": [{"_name": "USB2.0-Ser!", "vendor_id": "0x1a86"}]}
    monkeypatch.setattr(
        "brasa.core.port.subprocess.run", lambda *a, **kw: _make_profiler_result(data)
    )
    _usb_tree_fallback()
    err = capsys.readouterr().err
    assert "CH340" in err
...
def test_fallback_no_devices_suggests_cable(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
...
def test_fallback_timeout_handled(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
